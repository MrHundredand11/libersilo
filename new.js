        ...
        .then(data => {
            ...
            function genMen(data) {
                ...
                for (mn in data.record.scions) {
                    var mnu = createElementWithClass('span', 'styl_menu_entry');
                    mnu.innerHTML = data.record.scions[mn].name + "<br>";
                    var f_test = "fillErUp('" + data.record.scions[mn].url + "')";
                    mnu.setAttribute('onclick', f_test);
                    menu_tablet.appendChild(mnu);
                }
...
function gen_lf(lf_i, scion_id, lf_type, lf_term, lf_tag, lf_cntn, bld_itr, lf_trm, leaflets) {
    switch (lf_type) {
        case 'li_entr':
            ...
            lf_cntnr.appendChild(lf_pnt);
            lf_cntnr.appendChild(lf_trm);
            return lf_cntnr;
                    ...
                    case 'blade':
                        ...
                        if (bld_term !== undefined) {
                            ...
                            var lf_eqls = createElementWithClass('span', 'styl_pointers');
                            ...
                            switch (stems[i2].insc_type){
                                case 'stm_list':
                                    ...
                                    for (i3 in stems[i2].scions) {
                                        var entr_full = gen_lf(i3, stems[i2].scion_id, stems[i2].scions[i3].insc_type, stems[i2].scions[i3].insc_term, stems[i2].scions[i3].insc_tag, stems[i2].scions[i3].insc_cntn);
