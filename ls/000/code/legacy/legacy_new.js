...
function gen_lf(lf_i, scion_id, lf_type, lf_term, lf_tag, lf_cntn, leaflets) {
    switch (lf_type) {
        case 'li_entr':
        ...
        case 'li_cite':
            ...
            return lf_cntnr;
                    ...
                    case 'blade':
                        ...
                        var bld_tag = generateTag(bld_id, lf_tag);
                        ...
                        if (leaflets !== undefined) {
                            var leaflet_trestleboard = createElementWithClass('ul', 'styl_leaf_trst');   
                            for (lft in leaflets) {
                                var leafl = "";
                                function f (lf){
                                    var leaf_cntn = leaflets[lft].leaf_cntn;
                                    var leafl = gen_leaflet(lf, leaf_cntn);
                                    let leafl_i = document.createRange().createContextualFragment(leafl);
                                    return leafl;
                                }
                                var testes = f(leafl);
                                leaflet_trestleboard.appendChild(testes);
                            }
                            blade_full.appendChild(leaflet_trestleboard);
                        }
                        //return blade_full;
                        break;
                    ...
                    case 'stm_olst_exp':
                        ...
                        var lf_cntnr = createElementWithClass('div', 'styl_lf_stm_olst_exp');
                        var lf_bmp_top = createElementWithClass('div', 'styl_lf_stm_olst_exp_bmp_top');
                        var lf_ttl = createElementWithClass('div', 'styl_lf_stm_olst_exp_ttl');
                        var lf_cntn = createElementWithClass('div', 'styl_lf_stm_olst_exp_cntn');
                        var lf_ls_shrt = createElementWithClass('div', 'styl_lf_stm_olst_exp_ls_shrt');
                        var lf_bmp_cycl = createElementWithClass('div', 'styl_lf_stm_olst_exp_bmp_cycl');
                        var lf_expnd = createElementWithClass('div', 'styl_lf_stm_olst_exp_expnd');
                        var lf_bmp_btm = createElementWithClass('div', 'styl_lf_stm_olst_exp_bmp_btm');

                        lf_cntnr.setAttribute('id', `${lf_id}_stm_olst_exp`);
                        lf_bmp_top.setAttribute('id', `${lf_id}_stm_olst_exp_bmp_top`);
                        lf_ttl.setAttribute('id', `${lf_id}_stm_olst_exp_ttl`);
                        lf_cntn.setAttribute('id', `${lf_id}_stm_olst_exp_cntn`);
                        lf_ls_shrt.setAttribute('id', `${lf_id}_stm_olst_exp_ls_short`);
                        lf_bmp_cycl.setAttribute('id', `${lf_id}_stm_olst_exp_bmp_cycl`);
                        lf_expnd.setAttribute('id', `${lf_id}_lf_stm_olst_exp_expnd`);
                        lf_bmp_btm.setAttribute('id', `${lf_id}_lf_stm_olst_exp_bmp_btm`);
                        lf_bmp_top.innerHTML = "--- --- --- --- --- --- --- ||| --- --- --- --- --- --- ---";
                        lf_bmp_cycl.innerHTML = "------- ------- -------<br>------- -------<br>-------";
                        lf_ttl.innerHTML = `${pnt_branch}&nbsp${lf_term}`;
                        lf_cntnr.appendChild(lf_bmp_top);
                        lf_cntnr.appendChild(lf_ttl);
                        lf_cntnr.appendChild(lf_cntn);
                        lf_cntn.appendChild(lf_ls_shrt);
                        lf_cntn.appendChild(lf_bmp_cycl);
                        lf_cntn.appendChild(lf_expnd);
                        lf_cntnr.appendChild(lf_bmp_btm);

                        //console.log(leaflets);

                        for (lf in leaflets) {
                            console.log(leaflets[lf].insc_term);
                            var lfl_shrt = createElementWithClass('div', 'styl_lfl_shrt');
                            
                            lfl_shrt.innerHTML = leaflets[lf].insc_term;
                            ...
                            lfl_ttl.innerHTML = `+=>&nbsp${leaflets[lf].insc_term}&nbsp::`;
